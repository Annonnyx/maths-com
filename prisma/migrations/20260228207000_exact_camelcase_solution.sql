...
-- Les colonnes gardent EXACTEMENT leurs noms Prisma (camelCase)

-- Extension UUID
CREATE EXTENSION IF NOT EXISTS "uuid-ossp";

-- Tables avec noms Prisma EXACTS (camelCase)
...
-- Politiques RLS : Utiliser des noms de colonnes en minuscules sans guillemets
-- PostgreSQL a créé les colonnes en minuscules (userId → userid)
...
CREATE POLICY questions_own ON questions
    FOR ALL
    USING (testid IN (
        SELECT id FROM tests WHERE userid = auth.uid()::text
    ))
...
CREATE POLICY multiplayer_games_own ON multiplayer_games
    FOR ALL
    USING (player1id = auth.uid()::text OR player2id = auth.uid()::text)
    WITH CHECK (player1id = auth.uid()::text OR player2id = auth.uid()::text);

CREATE POLICY multiplayer_questions_own ON multiplayer_questions
    FOR ALL
    USING (gameid IN (
        SELECT id FROM multiplayer_games
        WHERE player1id = auth.uid()::text OR player2id = auth.uid()::text
    ))
...
CREATE POLICY friendships_own ON friendships
    ...
    WITH CHECK (user1id = auth.uid()::text OR user2id = auth.uid()::text);

CREATE POLICY messages_own ON messages
    FOR ALL
    USING (senderid = auth.uid()::text OR receiverid = auth.uid()::text)
    WITH CHECK (senderid = auth.uid()::text OR receiverid = auth.uid()::text);

CREATE POLICY challenges_own ON challenges
    FOR ALL
    USING (challengerid = auth.uid()::text OR challengedid = auth.uid()::text)
    WITH CHECK (challengerid = auth.uid()::text OR challengedid = auth.uid()::text);
...
CREATE POLICY class_groups_teacher_own ON class_groups
    FOR ALL
    USING (teacherid = auth.uid()::text)
    WITH CHECK (teacherid = auth.uid()::text);

CREATE POLICY class_groups_member_view ON class_groups
    FOR SELECT
    USING (EXISTS (
        SELECT 1 FROM class_group_members
        WHERE groupid = class_groups.id AND userid = auth.uid()::text
    ));
...
CREATE POLICY class_messages_member_view ON class_messages
    ...
    USING (EXISTS (
        SELECT 1 FROM class_group_members
        WHERE groupid = class_messages.groupid AND userid = auth.uid()::text
    ));
...
-- Pour faq_submissions, ajouter la colonne userId en minuscules (sans guillemets)
ALTER TABLE faq_submissions ADD COLUMN userid TEXT REFERENCES users(id);

CREATE POLICY faq_submissions_own ON faq_submissions
    FOR INSERT
    WITH CHECK (auth.uid()::text = COALESCE(userid, 'anonymous'));
