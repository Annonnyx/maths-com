...
-- Étape 3: Créer les politiques RLS avec cast correct pour auth.uid()
-- auth.uid() retourne un UUID, nous devons le convertir en TEXT pour la comparaison
...
CREATE POLICY faq_submissions_own ON faq_submissions
    FOR ALL
    USING (auth.uid()::text = COALESCE(userId, 'anonymous'))
